...
# bs4.BeautifulSoup() #BeautifulSoup = 클래쓰 = 대문자가 있는 이유
...
#code list에 있는 회사들 정리하기 for문 활용해서
code_list = ['005930', '035720', '003490']
name_list = ['삼성전자','카카오','대한항공']
...
for index in range(len(code_list)) :
    ...
    span_code = doc.select('span.code') #결과를 리스트로
    code = code_list[index]   #종목 코드
    span_blind = doc.select('span.blind')  #span.blind를 리스트로
    name = name_list[index]   #종목 이름
